...
const europassToDatabase = async (uri) => {
  try {
    ...
    const json = JSON.parse(JSON.stringify(convert.xml2js(xml, { compact: true })));
    ...
    const query = {
      ...
      args: [
        json['Candidate']['CandidatePerson']['PersonName']['oa:GivenName']['_text'],
        json['Candidate']['CandidatePerson']['PersonName']['hr:FamilyName']['_text'],
